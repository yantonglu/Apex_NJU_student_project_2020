...
static RegisterPass<ExpandConstantExpr> X("constantexpr", "Hello World Pass",false,true);
// Based on ConstantExpr::getAsInstruction() in lib/VMCore/Constants.cpp.
//  Use getAsInstruction() when we require a newer LLVM version.

static Instruction *getConstantExprAsInstruction(ConstantExpr *CE,Instruction *InsertPt) 
{
    ...
    else
{
      ...
      return GetElementPtrInst::Create(Ops[0]->getType(),Ops[0], Ops.slice(1));
  ...
  case Instruction::FCmp:
    ...
    return CmpInst::Create((Instruction::OtherOps)CE->getOpcode(),
                           ...
                           (enum llvm::CmpInst::Predicate)CE->getPredicate(), Ops[0], Ops[1]);
  ...
  default:
	...
	BinaryOperator *BO =
	BinaryOperator::Create((Instruction::BinaryOps)CE->getOpcode(),Ops[0], Ops[1]);
	if (OverflowingBinaryOperator *Op =dyn_cast<OverflowingBinaryOperator>(BO)) {
...
bool ExpandConstantExpr::runOnFunction(Function &func) {
  ...
  for (llvm::Function::iterator bb = func.begin();
    ...
    for (BasicBlock::InstListType::iterator inst = bb->begin();
      ...
      modified |= expandInstruction(cast<Instruction>(inst));
